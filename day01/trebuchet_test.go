...
func TestTrebuchetOnlyDigit(t *testing.T) {
	var tests = []struct {
		input    []string
		expected int
	}{
		{[]string{"1abc2", "pqr3stu8vwx", "a1b2c3d4e5f", "treb7uchet"}, 142},
		{actualTestInput, 53974}}
	for _, test := range tests {
		got := TrebuchetOnlyDigit(test.input)
		if got != test.expected {
			t.Errorf("[TestTrebuchetOnlyDigit] Got(%d) expected(%d)", got, test.expected)
		}
	}
}

func TestTrebuchetDigitAndLetter(t *testing.T) {
	tests := []struct {
		input    []string
		expected int
	}{
		...
		{actualTestInput, 52840}}
	for _, test := range tests {
		got := TrebuchetDigitAndLetter(test.input)
		if got != test.expected {
			t.Errorf("[TestTrebuchetDigitAndLetter] Got(%d) expected(%d)", got, test.expected)
		}
	}
}
