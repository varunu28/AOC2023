...
func TestGetSumOfPartNumbers(t *testing.T) {
	var tests = []struct {
		...
		{sampleTestInput, 4361},
		{actualTestInput, 509115},
	}
	for _, test := range tests {
		if output := GetSumOfPartNumbers(test.input); output != test.expected {
			t.Errorf("[TestGetSumOfPartNumbers] Expected(%d) Received(%d)", test.expected, output)
		}
	}
}
...
func TestGetSumOfGearRatios(t *testing.T) {
	var tests = []struct {
		...
		{sampleTestInput, 467835},
		{actualTestInput, 75220503},
	}
	for _, test := range tests {
		if output := GetSumOfGearRatios(test.input); output != test.expected {
			t.Errorf("[TestGetSumOfGearRatios] Expected(%d) Received(%d)", test.expected, output)
		}
	}
}
