...
func TestGetValidGamesIdSum(t *testing.T) {
	var tests = []struct {
		input    []string
		expected int
	}{
		{sampleTestInput, 8},
		{actualTestInput, 2285}}
	for _, test := range tests {
		got := GetValidGamesIdSum(test.input)
		if got != test.expected {
			t.Errorf("[TestGetValidGamesIdSum] Got(%d) expected(%d)", got, test.expected)
		}
	}
}

func TestGetSumOfPowerOfGame(t *testing.T) {
	var tests = []struct {
		input    []string
		expected int
	}{
		{sampleTestInput, 2286},
		{actualTestInput, 77021}}
	for _, test := range tests {
		got := GetSumOfPowerOfGame(test.input)
		if got != test.expected {
			t.Errorf("[TestGetSumOfPowerOfGame] Got(%d) expected(%d)", got, test.expected)
		}
	}
}
