...
func TestCountStepsForFarthestPoint(t *testing.T) {
	var tests = []struct {
		input    []string
		expected int
	}{
		{sampleTestInput, 4},
		{actualTestInput, 6599},
	}
	for _, test := range tests {
		if output := CountStepsForFarthestPoint(test.input); output != test.expected {
			t.Errorf("[TestCountStepsForFarthestPoint] Expected(%d) Received(%d)", test.expected, output)
		}
	}
}

func TestCountTilesEnclosedByLoop(t *testing.T) {
	var tests = []struct {
		input    []string
		expected int
	}{
	...
	for _, test := range tests {
		if output := CountTilesEnclosedByLoop(test.input); output != test.expected {
			t.Errorf("[TestCountTilesEnclosedByLoop] Expected(%d) Received(%d)", test.expected, output)
		}
	}
}
